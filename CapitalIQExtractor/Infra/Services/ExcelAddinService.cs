...
namespace CapitalIQExtractor.Infra.Services
{
    public class ExcelAddinService : IExcelAddinService
    {
        ...
        private const int SW_HIDE = 0; // Constante para esconder a janela
        ...
        private static Excel.Application GetExcelInterop(int? processId = null)
        {
            ...
            try
            {
                var app = new ExcelAddinService().SearchExcelInterop(p);

                // Esconder a janela do Excel
        ...
        public async Task<Excel.Application> GetExcelInstance()
        {
            var excel = await Task.Run(() =>
            {
                ...
                instance.Visible = false; // Garantir que a interface não é visível
                return instance;
            });

            // Desabilitar alertas e outras configurações para deixar o Excel mais rápido
